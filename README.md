...
管理者は、Firebase Authentication を使ってログインします。  
新規登録時には確認メールを送信し、メール確認が完了していない場合はログインできません。

### 組織管理

管理者は複数の組織を作成・選択できます。  
組織ごとに6桁の組織IDが発行され、従業員ログインに使用されます。

### 従業員ログイン

従業員は、管理者から共有された組織IDと、登録済みのメールアドレスでログインできます。  
従業員側ではアカウント作成やパスワード登録は不要です。
...
従業員IDは自動発行され、希望シフトや勤務履歴と紐づきます。

### ポジション管理

シフト募集時に使用するポジションを登録できます。
...
登録したポジションは、シフト枠作成や従業員の自主希望で使用されます。

### シフト枠管理

管理者は、日付・開始時間・終了時間・ポジション・募集人数を指定してシフト枠を作成できます。

また、同じ曜日・時間・ポジションのシフト枠を月内でまとめて作成することもできます。

### 希望シフト管理

従業員が提出した希望シフトを、管理者が確認・承認できます。  
承認済みのシフトは、従業員のマイカレンダーや勤務時間・給与計算に反映されます。

### おすすめ候補

シフト枠ごとに、提出された希望の中からおすすめ候補を表示します。

おすすめ候補は、以下をもとに計算されます。
...
相性スコアは、従業員同士が入力した双方向の評価を平均して使います。  
業務スキルは、管理者が従業員ごとに設定したスコアを使います。

### 公平性スコア

公平性スコアを有効にすると、勤務時間の偏りが少なくなるように候補を選びます。  
まず勤務時間が少ない従業員を優先し、同じ条件の場合に相性スコアや業務スキルを参考にします。

### 自動承認

管理者は、おすすめ候補を自動で承認するルールを設定できます。
...
- 自動承認する対象期間
- 確定タイミング
- 管理者作成枠のみ対象にするか、自主希望も含めるか

### 募集枠なしの自主希望

管理者が許可している場合、従業員は管理者が作成したシフト枠がない時間でも希望を提出できます。
...
この機能により、管理者が事前に枠を作っていない時間帯でも、従業員の勤務希望を集められます。
...
また、補正内容を従業員画面に表示するかどうかも設定できます。

### 従業員のマイカレンダー

従業員は、自分の確定シフトを月ごとに確認できます。  
日付を選択すると、その日のシフトがタイムライン形式で表示されます。

### エクスポート

#### 管理者

管理者は、承認済みシフトを以下の形式で出力できます。
...
従業員は、自分の確定シフトを以下の形式で出力できます。

- PNG
- ICS
- ICSカレンダー購読URL

カレンダー購読URLを使うことで、スマートフォンやPCのカレンダーアプリにシフトを追加できます。

## 設定画面

管理者は、設定画面から以下を変更できます。
...
- 商用化する場合は、本体リポジトリを private にすることを推奨します
