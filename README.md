...
| 管理者ログイン情報の保存 | 「ログイン情報を保存する」を選択すると、メールアドレスのみをブラウザの `localStorage` に保存し、ログイン成功時から30日間、次回のログイン画面で自動入力します。期限内に再ログインすると保存期限はその時点から更新されます。パスワードはアプリに保存せず、ブラウザ／OSのパスワードマネージャーに任せます。チェックを外すと保存情報だけを削除し、入力欄の内容は保持します。 |
...
| 従業員ログイン情報の保存 | 「ログイン情報を保存する」を選択すると、組織IDとメールアドレスをブラウザの `localStorage` に保存し、ログイン成功時から30日間、次回のログイン画面で自動入力します。期限内に再ログインすると保存期限はその時点から更新されます。チェックを外すと保存情報だけを削除し、入力欄の内容は保持します。 |
